...
import (
	...
	os_util "github.com/synesissoftware/libpath.Go/util/unix"
...
func simplePathSplitFully(path string) ([]string, error) {
	return angols_strings.SplitAfterByte(path, os_util.PathElementSeparator), nil
}
	...
	for _, s := range elems {
		if 0 == len(s) {
			...
			if !last_has_trailing && 0 != b.Len() {
				b.WriteByte(os_util.PathElementSeparator)
			}
			b.WriteString(s)
...
func createPathDescriptor(path string, ref_dir string, parseFlags types.ParseFlags) (types.PathDescriptor, error) {
	...
	case types.DriveLetterRelative, types.DriveLetterRooted, types.UncIncomplete, types.UncRooted:

		panic(fmt.Errorf("invalid classification %s in Unix API", &classification))
	case types.Relative, types.SlashRooted, types.HomeRooted:

		if types.Relative == classification && 0 != len(ref_dir) {
			fullPath := simplePathJoin(ref_dir, path)

			return createPathDescriptor(fullPath, "", parseFlags)
		}
...
func ClassifyRoot(path string, parseFlags types.ParseFlags) (types.Classification, string, string, int) {
	...
	if path[0] == os_util.PathElementSeparator {
		return types.SlashRooted, "/", path[1:], -1
	}
